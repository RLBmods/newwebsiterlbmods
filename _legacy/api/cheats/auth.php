...
class api
{
    public $name, $ownerid, $secret;
    public $numUsers, $numKeys, $numOnlineUsers, $customerPanelLink;

    function __construct($name, $ownerid, $secret)
    {
        $this->name = $name;
    ...
    function init()
    {
        if ($this->name == "" || $this->ownerid == "" || $this->secret == "") {
            die("Please set your application name, ownerid and secret in credentials.php");
        }
    ...
    function Rebrand($name, $level)
    {
        $data = array(
            ...
            if(!$json)
            {
                return array(
                    "success"=> false,
                    "message" => "Invalid JSON response from auth"
                    );
            }
    ...
    {
        $curl = curl_init("https://fireauthenticator.com/api/1.2/");
