...
try {
    ...
    echo json_encode([
        'success' => false,
        'message' => 'This product does not need to be reset.'
    ]);
    break;
case 'valorant':
            // API Configuration
            ...
            try {
                ...
                // Check success flag or fallback to HTTP 200 check
                $isSuccess = ($responseBody['success'] ?? false) === true || $response->getStatusCode() === 200;
                $message = $responseBody['message'] ?? 'HWID Reset processed successfully.';
